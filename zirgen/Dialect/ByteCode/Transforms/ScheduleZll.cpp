...
struct ZllBufferize : public BufferizeInterface {
  ZllBufferize(Liveness& liveness) : liveness(liveness) {}

  void analyze(Region* region);

  DenseMap<Value, size_t> valIndex;

  std::pair</*intKind=*/mlir::Attribute, /*index=*/size_t>
  getKindAndIndex(mlir::Value value) override {
    return std::make_pair(getIntKind(value), getValueIndex(value));
  }

  size_t getValueIndex(mlir::Value value) { return valIndex.lookup(value); }
  Attribute getIntKind(mlir::Value value) {
    auto name = TypeSwitch<Type, StringRef>(value.getType())
                    ...
                    .Case<ConstraintType>([&](auto constraintType) { return "LocalFpExt"; })
                    .Default([&](auto) -> llvm::StringRef {
                      llvm::errs() << value << "\n";
                      assert(0 && "Unknown value type to bufferize");
                    });
    return StringAttr::get(value.getContext(), name);
  }

  Liveness& liveness;
};

void ZllBufferize::analyze(Region* region) {
  llvm::DenseMap</*intKind=*/Attribute, /*next index=*/size_t> byType;
  llvm::DenseMap</*intKind=*/Attribute, SmallVector<size_t>> deadByType;
  llvm::DenseMap<Value, /*index=*/size_t> active;

  for (Operation* op : llvm::make_pointer_range(region->front())) {
    for (Value result : op->getResults()) {
      Attribute intKind = getIntKind(result);
      assert(!active.contains(result));
      auto& dead = deadByType[intKind];
      size_t index;
      if (dead.empty()) {
        index = byType[intKind]++;
      } else {
        index = dead.pop_back_val();
      }
      valIndex[result] = index;
      active[result] = index;
    }
    ...
    for (Value operand : op->getOperands()) {
      if (liveness.isDeadAfter(operand, op)) {
        if (!active.contains(operand))
          continue;
        deadByType[getIntKind(operand)].push_back(active[operand]);
        active.erase(operand);
      }
    }
  }
