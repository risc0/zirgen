...
StringRef getLayoutBuffer(GlobalConstOp globalConstOp) {
  ...
  if (symName.ends_with("$accum")) return "accum";
  if (symName == "Top") return "data";
  if (symName.starts_with("test$")) return "test";
  return symName;
}
...
BufferAnalysis::getLayoutAndBufferForArgument(mlir::BlockArgument layoutArg) {
  if (!ZStruct::isLayoutType(layoutArg.getType())) return {};
