...
struct InlineLayout : public OpRewritePattern<BindLayoutOp> {
  ...
  LogicalResult matchAndRewrite(BindLayoutOp op, PatternRewriter& rewriter) const final {
    auto layoutConst = llvm::dyn_cast<SymbolRefAttr>(op.getLayout());
    if (!layoutConst)
      return rewriter.notifyMatchFailure(op, "already inlined");

    auto globConst = ModuleOp(mod).lookupSymbol<GlobalConstOp>(layoutConst);
    if (!globConst)
      return rewriter.notifyMatchFailure(op, "Undefined layout reference");

    rewriter.modifyOpInPlace(op, [&]() { op.setLayoutAttr(globConst.getConstantAttr()); });
    return success();
  }
