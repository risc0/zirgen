...
struct TapMap {
  TapMap(CircuitDefOp circuitDef) : circuitDef(circuitDef) {}
  ...
  uint32_t getRegGroupId(BlockArgument ba) {
    ...
    for (auto buf : circuitDef.getBuffers().getAsRange<BufferDefAttr>()) {
      if (buf.getName() == name) {
...
struct ComputeTapsPass : public ComputeTapsBase<ComputeTapsPass> {
  void runOnOperation() override {
    ...
    TapMap tapMap(CircuitDefOp::lookupInModule(mod));
