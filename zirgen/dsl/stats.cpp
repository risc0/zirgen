...
struct StatsPrinter {
  ...
  /*  void printDetailedConstraints(Zhlt::CheckFuncOp check, size_t totalConstraints) {
      DenseMap<Attribute, size_t> counts;
      Attribute star = UnknownLoc::get(check.getContext());

      check.walk([&](Zll::EqualZeroOp eqzOp) {
        DenseSet<Attribute> locDedup;

        LocationAttr topLoc = eqzOp.getLoc();
        LocationAttr(topLoc).walk([&](LocationAttr subLoc) -> WalkResult {
          AttrTypeReplacer replacer;
          replacer.addReplacement([&](Attribute attr) -> Attribute {
            if (attr == subLoc) {
              return star;
            } else {
              return attr;
          ...
          locDedup.insert(replaced);
  //        llvm::errs() << "Replacing in: " << topLoc << "\nSearch: " << subLoc << "\nReplaced: "
  << replaced << "\n"; return WalkResult::advance();
        });

        for (auto loc : locDedup) {
          counts[loc]++;
        }
      });
