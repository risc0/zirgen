...
class WeierstrassCurve {
  ...
  //  y^2 = x^3 + a*x + b  (mod p)
public:
  WeierstrassCurve(APInt a_coeff, APInt b_coeff, APInt prime)
      : _a_coeff(a_coeff), _b_coeff(b_coeff), _prime(prime){};
  ...
  Value a_as_bigint(OpBuilder builder, Location loc) const {
    mlir::Type type = builder.getIntegerType(
        _a_coeff.getBitWidth()); // TODO: I haven't thought through signedness
  ...
  Value b_as_bigint(OpBuilder builder, Location loc) const {
    mlir::Type type = builder.getIntegerType(
        _b_coeff.getBitWidth()); // TODO: I haven't thought through signedness
  ...
  Value prime_as_bigint(OpBuilder builder, Location loc) const {
    mlir::Type type =
        builder.getIntegerType(_prime.getBitWidth()); // TODO: I haven't thought through signedness
...
public:
  AffinePt(Value x_coord, Value y_coord, std::shared_ptr<WeierstrassCurve> curve, Value order)
      : _x(x_coord), _y(y_coord), _curve(curve), _order(order){};
...
AffinePt
mul(OpBuilder builder, Location loc, Value scalar, const AffinePt& pt, const AffinePt& arbitrary);
...
void makeECAffineValidatePointsEqualTest(mlir::OpBuilder builder,
                                         ...
                                         APInt curve_a,
                                         APInt curve_b);
// void makeECAffineValidatePointOnCurveTest(mlir::OpBuilder builder, mlir::Location loc, size_t
// bits);
