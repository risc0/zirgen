...
void WeierstrassCurve::validate_contains(OpBuilder builder,
  ...
  // y_sqr = builder.create<BigInt::ReduceOp>(loc(),  y_sqr, prime);  // TODO: Better to skip reduce
  // here and do it on the difference?

  Value x_cube = builder.create<BigInt::MulOp>(loc(), pt.x(), pt.x());
  x_cube = builder.create<BigInt::ReduceOp>(
      loc(), x_cube, prime); // TODO: Reduce isn't required for correctness, perf better if skipped?
                             // // TODO: But seems to make an overflow if dropped?
  x_cube = builder.create<BigInt::MulOp>(loc(), pt.x(), x_cube);
  // x_cube = builder.create<BigInt::ReduceOp>(loc(),  x_cube, prime);  // TODO: Reduce isn't
  // required for correctness, perf better if skipped?

  Value ax = builder.create<BigInt::MulOp>(loc(), a_as_bigint(builder, loc()), pt.x());
  // ax = builder.create<BigInt::ReduceOp>(loc(),  ax, prime);  // TODO: Reduce isn't required for
  // correctness, perf better if skipped?

  Value weierstrass_rhs = builder.create<BigInt::AddOp>(loc(), x_cube, ax);
  weierstrass_rhs =
      builder.create<BigInt::AddOp>(loc(), weierstrass_rhs, b_as_bigint(builder, loc()));
  // weierstrass_rhs = builder.create<BigInt::ReduceOp>(loc(),  weierstrass_rhs, prime);  // TODO:
  // Better to skip reduce here and do it on the difference?

  Value diff = builder.create<BigInt::SubOp>(loc(), y_sqr, weierstrass_rhs);
  diff = builder.create<BigInt::ReduceOp>(
      loc(), diff, prime); // TODO: Testing doing here instead of on its inputs
  builder.create<BigInt::EqualZeroOp>(loc(), diff);
}

void AffinePt::validate_equal(OpBuilder builder, Location _loc, const AffinePt& other) const {
  AutoSourceLoc loc(builder.getContext());
...
void AffinePt::validate_on_curve(OpBuilder builder, Location _loc) const {
  AutoSourceLoc loc(builder.getContext());
  _curve->validate_contains(builder, loc(), *this);
}

void AffinePt::validate_order(OpBuilder builder, Location _loc, const AffinePt& arbitrary) const {
  AutoSourceLoc loc(builder.getContext());
  ...
  // NOTE: If the claimed order is prime, this validates that the true order is _exactly_ the
  // claimed order This is because the true order must divide any scalar that sends the point to the
  // identity, and the order can't be 1 b/c the identity can't be represented in affine coords.
...
AffinePt add(OpBuilder builder, Location _loc, const AffinePt& lhs, const AffinePt& rhs) {
  AutoSourceLoc loc(builder.getContext());
  // Note: `add` can fail in two ways: A + A (doesn't use doubling algorithm) or A + (-A) (can't
  // write 0)  [TODO: Document?] Formulas (all mod `prime`):
  //   lambda = (yQ - yP) / (xQ - xP)
  //       nu = yP - lambda * xP
  //       xR = lambda^2 - xP - xQ
  //       yR = -(lambda * xR + nu)
  ...
  y_diff = builder.create<BigInt::AddOp>(
      loc(), y_diff, prime); // TODO: Reduce op doesn't work with negatives, so enforcing positivity
                             // // TODO: Can this be removed?
  // y_diff = builder.create<BigInt::ReduceOp>(loc(),  y_diff, prime);  // TODO: Not needed for
  // correctness, so can experiment with removing
  Value x_diff = builder.create<BigInt::SubOp>(loc(), rhs.x(), lhs.x());
  x_diff = builder.create<BigInt::AddOp>(
      loc(), x_diff, prime); // TODO: Reduce op doesn't work with negatives, so enforcing positivity
  // x_diff = builder.create<BigInt::ReduceOp>(loc(),  x_diff, prime);   // TODO: Can this be
  // removed? Investigate NondetInvMod
  ...
  lambda = builder.create<BigInt::ReduceOp>(
      loc(), lambda, prime); // TODO: Skipping this one breaks an assert; shouldn't be required for
                             // correctness but I think it overflows the coeffs

  Value nu = builder.create<BigInt::MulOp>(loc(), lambda, lhs.x());
  // nu = builder.create<BigInt::ReduceOp>(loc(),  nu, prime);  // TODO: Reduce isn't required for
  // correctness, perf better if skipped?
  nu = builder.create<BigInt::SubOp>(loc(), lhs.y(), nu);
  nu = builder.create<BigInt::AddOp>(
      loc(), nu, prime); // TODO: Reduce op doesn't work with negatives, so enforcing positivity
  nu = builder.create<BigInt::ReduceOp>(loc(), nu, prime);

  Value xR = builder.create<BigInt::MulOp>(loc(), lambda, lambda);
  // xR = builder.create<BigInt::ReduceOp>(loc(),  xR, prime);  // TODO: Not needed for correctness,
  // so can experiment with removing
  xR = builder.create<BigInt::SubOp>(loc(), xR, lhs.x());
  xR = builder.create<BigInt::AddOp>(
      loc(), xR, prime); // TODO: Reduce op doesn't work with negatives, so enforcing positivity
  xR = builder.create<BigInt::SubOp>(loc(), xR, rhs.x());
  xR = builder.create<BigInt::AddOp>(
      loc(), xR, prime); // TODO: Reduce op doesn't work with negatives, so enforcing positivity  //
                         // TODO: Merge the 2 adds? Not without constant propagation
  ...
  yR = builder.create<BigInt::AddOp>(
      loc(), yR, prime); // TODO: Reduce op doesn't work with negatives, so enforcing positivity  //
                         // TODO: better with using 2*prime for sub?
  yR = builder.create<BigInt::ReduceOp>(loc(), yR, prime);
...
AffinePt
mul(OpBuilder builder, Location _loc, Value scalar, const AffinePt& pt, const AffinePt& arbitrary) {
  AutoSourceLoc loc(builder.getContext());
  ...
  // No separate testing is needed, as this will compute the `result` value (inclusive of
  // `arbitrary`) to be equal to `arbitrary` in this case, and then compute `sub` of `arbitrary`
  // with itself, which fails (via attempted division by zero) in `sub`.
  ...
  // This can fail if choosing an unlucky point Arb, but no soundness issues and can adjust Arb to
  // get completeness
  auto result = arbitrary;

  // The repeatedly doubled point
  auto doubled_pt = pt;
  // TODO: If we call this multiple times with the same `arbitrary`, then `validate_on_curve` will
  // be called multiple times for it I think this is OK as CSE removes everything except the final
  // eqz, and this doesn't get called _that_ often
  // TODO: Perhaps have CSE remove duplicate eqz's too?
  // TODO: Removing this slightly reduces the cycle count, so the CSE isn't perfect here. Possible
  // location for minor perf gains NOTE: We assume `pt` is already validated as on the curve (either
  // manually already, or by construction) [TODO: Document better?] `arbitrary` has not need to be
  // constructed in any particular way, so we pretty much always need to validate it's on the curve
  // Hence, we do so here
  ...
  // Note that until we do this, the small tests will fail due to the extra collision opportunities
  // with Arbitrary
  llvm::outs() << "    EC mul with " +
                      std::to_string(llvm::cast<BigIntType>(scalar.getType()).getMaxBits()) +
                      " iterations\n"; // TODO: Temporary log
  for (size_t it = 0; it < llvm::cast<BigIntType>(scalar.getType()).getMaxBits();
       it++) { // TODO: Why is this slightly larger than the bitwidth?
  ...
  // TODO: Reduce negYArb? Shouldn't be needed if provided a point in 1 < * < prime, and I *think*
  // it's not _dangerous_ even if not
  return sub(builder, loc(), result, arbitrary);
}

AffinePt neg(OpBuilder builder, Location _loc, const AffinePt& pt) {
  AutoSourceLoc loc(builder.getContext());
  Value yR =
      builder.create<BigInt::SubOp>(loc(), pt.curve()->prime_as_bigint(builder, loc()), pt.y());
  return AffinePt(pt.x(), yR, pt.curve(), pt.order());
}

AffinePt doub(OpBuilder builder, Location _loc, const AffinePt& pt) {
  AutoSourceLoc loc(builder.getContext());
  ...
  lambda_num = builder.create<BigInt::MulOp>(
      loc(), three, lambda_num); // three is less than prime, so can add again before reduce
  lambda_num =
...
AffinePt sub(OpBuilder builder, Location _loc, const AffinePt& lhs, const AffinePt& rhs) {
  AutoSourceLoc loc(builder.getContext());
  // Note: `sub` can fail in two ways: A - A (can't write 0) or A - (-A) (doesn't use doubling
  // algorithm) [TODO: Document?]
  auto neg_rhs = neg(builder, loc(), rhs);
  return add(builder, loc(), lhs, neg_rhs);
}

void ECDSA_verify(OpBuilder builder,
                  Location _loc,
                  const AffinePt& base_pt,
                  const AffinePt& pub_key,
                  Value hashed_msg,
                  Value r,
                  Value s,
                  const AffinePt& arbitrary) {
  AutoSourceLoc loc(builder.getContext());
  pub_key.on_same_curve_as(base_pt);
  arbitrary.on_same_curve_as(base_pt);

  // Note: we don't need to validate `base_pt` on the curve as it's a publicly pre-committed
  // parameter whose presence on the curve we can verify ahead of time (much like we can verify the
  // order of the curve ahead of time)
  ...
  assert(pub_key.order() ==
         base_pt.order()); // TODO: I... think this isn't right, I think it's comparing _Values_
                           // _outside_ the circuit (TODO Check)
  // Note: We don't need to validate `base_pt`'s order as it's a publicly pre-committed parameter
  // whose order we can verify ahead of time
  pub_key.validate_order(builder, loc(), arbitrary);
  // Mathematically, we need to ensure also that `pub_key` is not the identity.
  // But it's not possible to express the identity in affine coords, so this comes for free just by
  // validating the point is on the curve
  ...
  // Compute s_inv
  Value s_inv = builder.create<BigInt::NondetInvModOp>(
      loc(), s, base_pt.order()); // TODO: Do we need to handle `order` specially?
  Value s_inv_check = builder.create<BigInt::MulOp>(loc(), s, s_inv);
  s_inv_check = builder.create<BigInt::ReduceOp>(
      loc(), s_inv_check, base_pt.order()); // TODO: Do we need to handle `order` specially?
  ...
  u1 = builder.create<BigInt::ReduceOp>(
      loc(), u1, base_pt.order()); // TODO: Do we need to handle `order` specially?
  Value u2 = builder.create<BigInt::MulOp>(loc(), r, s_inv);
  u2 = builder.create<BigInt::ReduceOp>(
      loc(), u2, base_pt.order()); // TODO: Do we need to handle `order` specially?
...
void makeECDSAVerify(mlir::OpBuilder builder,
                     mlir::Location _loc,
                     size_t bits, // TODO: These `bits` parameters could maybe be inferred from the
                                  // prime (and definitely from the prime + the order)
                     APInt prime,
                     APInt curve_a,
                     APInt curve_b
                     /* TODO*/
  ...
  // Since curve order differs from prime by at most 2 * sqrt(prime), we only need 1 more bit than
  // `prime`
  auto order_bits = bits + 1;
  auto base_pt_X = builder.create<BigInt::DefOp>(
      loc(), bits, 0, true); // TODO: Or get from a parameter to this call?
  auto base_pt_Y = builder.create<BigInt::DefOp>(
      loc(), bits, 1, true); // TODO: Or get from a parameter to this call?
  auto base_pt_order = builder.create<BigInt::DefOp>(
  ...
  // TODO: Think through if we need to validate any of this (e.g. the orders, points being on
  // curves)
...
void makeECAffineAddTest(mlir::OpBuilder builder,
                         mlir::Location _loc,
                         size_t bits,
                         APInt prime,
                         APInt curve_a,
...
void makeECAffineDoubleTest(mlir::OpBuilder builder,
                            mlir::Location _loc,
                            size_t bits,
                            APInt prime,
                            APInt curve_a,
...
void makeECAffineMultiplyTest(mlir::OpBuilder builder,
                              mlir::Location loc_,
                              size_t bits,
                              APInt prime,
                              APInt curve_a,
  ...
  // auto order = builder.create<BigInt::DefOp>(loc(),  order_bits, 5, true);  // TODO: Or get from
  // a parameter to this call?  // TODO
  auto order = builder.create<BigInt::DefOp>(
      loc(), bits, 5, true); // TODO: Or get from a parameter to this call?
...
void makeECAffineNegateTest(mlir::OpBuilder builder,
                            mlir::Location _loc,
                            size_t bits,
                            APInt prime,
                            APInt curve_a,
...
void makeECAffineSubtractTest(mlir::OpBuilder builder,
                              mlir::Location _loc,
                              size_t bits,
                              APInt prime,
                              APInt curve_a,
...
void makeECAffineValidatePointsEqualTest(mlir::OpBuilder builder,
                                         mlir::Location loc_,
                                         size_t bits,
                                         APInt prime,
                                         APInt curve_a,
                                         APInt curve_b) {
  AutoSourceLoc loc(builder.getContext());
  // auto order_bits = bits + 1;  // Skip for this test, causing [11, 0] instead of [11] and
  // debugging that doesn't seem critical
...
// void makeECAffineValidatePointOrderTest(mlir::OpBuilder builder, mlir::Location loc, size_t
// bits); void makeECAffineValidatePointOnCurveTest(mlir::OpBuilder builder, mlir::Location loc,
// size_t bits);
