...
that exactly one of the entries of the selector is nonzero.
...
refer to its value in previous cycles. In order for this to work naturally, the
least common super must have the same layout in all arms, which is guaranteed by
the language and handled automatically by the compiler.

Furthermore, since at most one arm of the mux is active in a particular row of
the execution trace, subcomponents from other arms cannot be used. This means
that the same columns can be reused for different subcomponents on different mux
arms.
