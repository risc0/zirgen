...
use risc0_zkp::{
    ...
    field::{
        baby_bear::{self, BabyBearExtElem},
        Elem,
    },
};
...
use crate::{
    ...
    zirgen::circuit::{PoseidonStateLayout, LAYOUT_TOP},
};
...
impl Poseidon2State {
    fn new_start(mode: u32) -> Self {
        Self {
            ...
            next_state: STATE_POSEIDON_PAGING,
            mode,
            ..Default::default()
        }
    }
    ...
    fn new_ecall(state_addr: u32, buf_in_addr: u32, buf_out_addr: u32, bits_count: u32) -> Self {
        ...
        Self {
            ...
            next_state: STATE_POSEIDON_ENTRY,
            ..Default::default()
        }
    }
    ...
    fn step(&mut self, ctx: &mut Preflight, cur_state: &mut u32, next_state: u32, sub_state: u32) {
    ...
    fn rest(&mut self, ctx: &mut Preflight, final_state: u32) -> Result<()> {
...
pub(crate) struct Checksum {
    powers: [BabyBearExtElem; DIGEST_WORDS * 2 + 1],
    pub zcheck: BabyBearExtElem,
}

impl Checksum {
    pub(crate) fn new(nonce: &BabyBearExtElem) -> Self {
        let mut cur = BabyBearExtElem::ONE;
        let mut powers = [BabyBearExtElem::ZERO; DIGEST_WORDS * 2 + 1];
    ...
    pub(crate) fn clear(&mut self) {
        self.zcheck = BabyBearExtElem::ZERO;
    }
    ...
    pub(crate) fn add(&mut self, tx_kind: u32, idx: usize, txn: &RawMemoryTransaction) {
        ...
        self.zcheck += self.powers[2 * idx + 0] * BabyBearExtElem::from_u32(coeffs.0);
        self.zcheck += self.powers[2 * idx + 1] * BabyBearExtElem::from_u32(coeffs.1);
    }
}
