...
__device__ std::array<uint32_t, 2>
...
// Map + reduce support
template <typename T1, typename F, size_t N> __device__ auto map(std::array<T1, N> a, F f) {
...
auto __device__ map(std::array<T1, N> a, std::array<T2, N> b, F f) {
...
template <typename T1, typename T2, typename F, size_t N>
auto __device__ map(std::array<T1, N> a, const BoundLayout<T2>& b, F f) {
...
template <typename T1, typename T2, typename F, size_t N>
auto __device__ reduce(std::array<T1, N> elems, T2 start, F f) {
  T2 cur = start;
  for (size_t i = 0; i < N; i++) {
    cur = f(cur, elems[i]);
  }
...
auto __device__ reduce(std::array<T1, N> elems, T2 start, const BoundLayout<T3>& b, F f) {
  T2 cur = start;
  for (size_t i = 0; i < N; i++) {
...
__device__ std::array<Val, 5> extern_getMemoryTxn(ExecContext& ctx, Val addrElem) {
...
__device__ void extern_log(ExecContext& ctx, const std::string& message, std::vector<Val> vals) {
...
__device__ std::array<Val, 4> extern_divide(
...
__device__ std::array<Val, 2> extern_getMajorMinor(ExecContext& ctx) {
...
__device__ std::array<Val, 2> extern_nextPagingIdx(ExecContext& ctx) {
